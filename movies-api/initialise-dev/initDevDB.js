...
import favorites from './favorates'
import User from '../api/users/userModel';
import Movie from '../api/movies/movieModel';
import Favorate from '../api/favorates/favorateModel';
...
async function main() {
    ...
    await Favorate.collection.drop().catch(err => console.log('Favorate collection not found'));
    await User.create(users);
    await Movie.create(movies);
    await Favorate.create(favorites)
    console.log('Database initialised');
    console.log(`${users.length} users loaded`);
    console.log(`${movies.length} movies loaded`);
    console.log(`${favorites.length} movies loaded`);
